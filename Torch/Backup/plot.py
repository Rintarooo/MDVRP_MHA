...
from dataset import generate_data, load_json#, data_from_txt
...
def plot_route(data, pi, costs, title, idx_in_batch = 0):
	...
	layout = go.Layout(title = dict(text = f'<b>VRP{customer_xy.shape[0]} depot{depot_xy.shape[0]} {title}, Total Length = {cost:.3f}</b>', x = 0.5, y = 1, yanchor = 'bottom', xref = 'paper', yref = 'paper', pad = dict(b = 10)),#https://community.plotly.com/t/specify-title-position/13439/3
