...
class Env():
	...
	def get_mask(self, next_node, next_car):
		...
		is_next_depot = (self.car_cur_node == self.car_start_node).bool()#.sum(-1)
		...
		used_car = torch.eye(self.n_car, device = self.device)[next_car.squeeze(1)]
		# used_car: (batch, n_car)
		
		# car_used_demand = is_next_customer.long() * used_car * selected_demand
		# car_used_demand = is_next_customer[:,None].repeat(1,self.n_car) * used_car * selected_demand
		# car_used_demand = is_next_customer * used_car * selected_demand
